...
<li><strong>Pivot to associated people or organisations.</strong> Nobody exists in isolation online — look at who and what surrounds this identifier.</li>
...
A username or photo match alone is never identity confirmation. People reuse profile photos, usernames get recycled by different people over the years, and stock photos exist. Treat a single-source match as POSSIBLE until something independent — a second platform, a named relative, a detail only the real person would have — moves it up.
...
One identifier generates candidates. Two independent identifiers, searched together, help you tell which candidate is actually your target — a meaningfully different kind of search from anything in the three-query rule above.
...
- Stay inside whatever rules and scope apply — a CTF's rules of engagement, or your organisation's policy for a real case.
...
Two names appearing together repeatedly across otherwise unrelated pages — a comment thread, a tagged photo, a team roster — is a real signal worth following up.
...
A shared username doesn't guarantee a shared person. Common patterns — a name plus a birth year — get reused independently by different people. Corroborate with a second identifier before treating two accounts as the same person.
